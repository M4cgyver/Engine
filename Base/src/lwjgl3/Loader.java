...
public class Loader 
{
	...
	public ModleRaw loadToVAO(float[] positions, int size)
	{
		int vaoID = createVAO();
		this.storeDataInAttributeList(0, size, positions);
		unbindVAO();
		return new ModleRaw(vaoID, positions.length/size);
	}
	
	public ModleRaw loadToVAO(float[] positions)
	{
		int vaoID = createVAO();
		this.storeDataInAttributeList(0, 2, positions);
		unbindVAO();
		return new ModleRaw(vaoID, positions.length/2);
	}
